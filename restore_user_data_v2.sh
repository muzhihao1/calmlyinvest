...
# User details - using the UUID directly as user_id
USER_ID="8e82d664-5ef9-47c1-a540-9af664860a7c"
USER_EMAIL="279838958@qq.com"

echo "=== Starting data restoration for user $USER_EMAIL (UUID: $USER_ID) ==="

# Step 1: Check existing portfolios for this user
echo -e "\n1. Checking existing portfolios..."
PORTFOLIO_CHECK=$(curl -s -X GET \
  "$SUPABASE_URL/rest/v1/portfolios?user_id=eq.$USER_ID&select=*" \
  -H "apikey: $SUPABASE_ANON_KEY" \
  -H "Authorization: Bearer $SUPABASE_ANON_KEY")

echo "Portfolio check response: $PORTFOLIO_CHECK"

# Step 2: Create portfolio (using text UUID for user_id)
echo -e "\n2. Creating portfolio..."
PORTFOLIO_RESPONSE=$(curl -s -X POST \
  "$SUPABASE_URL/rest/v1/portfolios?select=*" \
  -H "apikey: $SUPABASE_ANON_KEY" \
  -H "Authorization: Bearer $SUPABASE_ANON_KEY" \
...
if [ -z "$PORTFOLIO_ID" ]; then
  echo "ERROR: Could not create or find portfolio."
  echo "Let's check the actual error and table structure..."
  
  # Try to understand the table structure
  echo -e "\nChecking portfolios table structure..."
  curl -s -X GET \
...
# Step 3: Create stock holdings
echo -e "\n3. Creating stock holdings..."
...
# Step 4: Create option holdings
echo -e "\n4. Creating option holdings..."
...
# Step 5: Create risk settings
echo -e "\n5. Creating risk settings..."
...
echo "Risk settings response: $RISK_RESPONSE"

echo -e "\n\n=== Data restoration complete ==="

# Step 6: Verify the data was created
echo -e "\n6. Verifying data creation..."
...
echo -e "\nStock Holdings (if portfolio was created):"
if [ ! -z "$PORTFOLIO_ID" ]; then
  curl -s -X GET \
    "$SUPABASE_URL/rest/v1/stock_holdings?portfolio_id=eq.$PORTFOLIO_ID&select=*" \
    -H "apikey: $SUPABASE_ANON_KEY" \
    -H "Authorization: Bearer $SUPABASE_ANON_KEY" | python3 -m json.tool
fi

echo -e "\nOption Holdings (if portfolio was created):"
if [ ! -z "$PORTFOLIO_ID" ]; then
  curl -s -X GET \
    "$SUPABASE_URL/rest/v1/option_holdings?portfolio_id=eq.$PORTFOLIO_ID&select=*" \
    -H "apikey: $SUPABASE_ANON_KEY" \
    -H "Authorization: Bearer $SUPABASE_ANON_KEY" | python3 -m json.tool
fi
...
echo -e "\nRisk Settings:"
curl -s -X GET \
  "$SUPABASE_URL/rest/v1/risk_settings?user_id=eq.$USER_ID&select=*" \
  -H "apikey: $SUPABASE_ANON_KEY" \
  -H "Authorization: Bearer $SUPABASE_ANON_KEY" | python3 -m json.tool